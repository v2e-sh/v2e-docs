...
> **Domains are placeholders.** URLs below use `int.example.com`; substitute your own
> internal domain. *(This reference deployment uses `v2e.sh`.)* The `10.1.x.x` addresses are
> literal — they're generated by the network config and identical on every deploy.
